...
class Policy_PPO_Continuous(PolicyBase):
    ...
    def _pi_loss_continuous(self, mu, logp_old, act, adv):
        ...
        if self.log_std is None:
            self.log_std = tf.Variable(name= 'log_std', initial_value= -0.5 * np.ones(self.num_actions, dtype=np.float32))

        logp = self.pi.gaussian_likelihood(act, mu, self.log_std)
        ...
        # Entropy term
        entropy = self.entropy(self.log_std)
